...
import torch
from analysis_settings import img_size_, mean, std, dynamic_img_batch_, dynamic_mask_batch_, \
    bb_img_path, dynamic_mask_lrs, base_mask_height_width_list_,\
    best_loss_, best_epoch_, iteration_epoch_, iteration_epoch_min_, patient_, \
    mask_optimizer_lr_, check_epoch_, base_mask_size_list_, \
    img_mask_root_path1_, original_img_path1_, result_path1_, pathology_path1_
...
def Dynamic_Mask_bb(ppnet):
    ...
    for batch_idx, (x, _) in enumerate(original_img_loader):
        ...
        output, min_distances, x_map = ppnet_multi(x)
        predicted_positive = []
        threshold = 0.1
        ...
        torch_x_output, torch_distances, torch_x_map = ppnet.push_forward(x)
        ...
        for i in range(x.shape[0]):
            ...
            for base_mask_num in range(len(base_mask_height_width_list)):
                for j in range(num_prototypes_per_class):
                    ...
                    d_mask = dynamic_maskpair.Dynamic_MaskPair(img_size=img_size, base_mask_size=base_mask_size_list[base_mask_num])
                    d_mask = d_mask.cuda()
                    original_min = torch_x_output[int(i), int(prototype_batch[i][j]), :]

                    min_activation = original_min.cuda()
                    ...
                    for epoch in range(iteration_epoch):
                        if ((epoch - best_epoch) > patient and epoch > iteration_epoch_min):
                            ...
                            x_mask = d_mask(x[i])
                            torch_x_mask_output, _, _ = ppnet.push_forward(x_mask)
                            mask_min = torch_x_mask_output[:, int(prototype_batch[i][j]), :]
                            mask_min_activation = mask_min[0, :].cuda()
                            ...
                            mean_l1_loss = mean_l1_loss.cuda()
...
def close_only(model):
    for p in model.module.features.parameters():
        p.requires_grad = False
    for p in model.module.add_on_layers.parameters():
        p.requires_grad = False
    for p in model.module.ones.parameters():
        p.requires_grad = False
    model.module.prototype_vectors.requires_grad = False
    model.module.last_layer.requires_grad = False
